	            ...
	            <div class="col-sm-12">
	                <div class="ibox">
	                    <div class="ibox-content">
	                    	<form class="form-horizontal m-t" id="dqTestForm" action="${basePath }/result/departmentResult/addDepartmentResult" method="post">
	                              ...
	                              <div id="vertical-timeline" class="vertical-container dark-timeline">
		                              <c:choose>
		                                ...
		                                <c:otherwise>
		                                	<c:forEach items="${departmentQuestionList }" var="dq" varStatus="statusQ">
	                                  			<input type="hidden" name="dqids" value="${dq.dqid }" />
	                                  			<input type="hidden" name="dqscales" value="${dq.dqscale }" />
		                                  		<div class="vertical-timeline-block">
			                                        <div class="vertical-timeline-icon navy-bg">
			                                        	<span>${statusQ.index+1 }</span> 
			                                        </div>
			                                        <div class="vertical-timeline-content">
			                                          <h3>${dq.dqbasic }&nbsp;&nbsp;<span class="badge badge-primary">(${dq.dqscale }%)</span></h3> 
		                                        	  <c:forEach items="${dq.departmentOptionList }" var="dop"  varStatus="statusO">
				                                        	<p class="text-muted">
					                                        	<label class="i-checks">
					                                        	<input type="radio" name="do${dq.dqid }" lonum="${dop.dolonum }" upnum="${dop.doupnum }" 
					                                        	class="form-control" required="" aria-required="true" value="${dop.doid }" 
					                                        	<c:if test="${dq.doid!=null&&dop.doid==dq.doid}"> checked="checked"</c:if>
					                                        	>
					                                        		${dop.donum }&nbsp;.&nbsp;<span style="font-size:16px;font-weight: 400;">[${dop.doname }]</span>&nbsp;${dop.dodescribe }.&nbsp;&nbsp;<span class="badge badge-primary">(${dop.dolonum }~${dop.doupnum }分)</span>
					                                        	</label>
				                                        	</p>
			                                        	</c:forEach>
			                                        	<div class="form-group">
							                                <div class="col-sm-3">
							                                    <div class="input-group m-b">
							                                        ...
							                                        <input name="drdweight${dq.dqid }" minlength="1" lonum="0" upnum="10" 
							                                        placeholder="请输入评分" maxlength="3" type="text" class="form-control score" 
	        	...
	        	errorPlacement: function(error, element) {  
	   		    ...
	   		    submitHandler: function() {
		   		    $.ajax({
		     			cache: false,
		     			type: "POST",
		     			url: "${basePath }/result/departmentResult/addDepartmentResult",	
		     			data: $("#dqTestForm").serialize(),	
		     			async: false,
		     			success: function(data) {
		     				swal({
		     					title:"部门测评",text:"测评完成!",
