	                    ...
	                    <div class="ibox-content">
								 <div class="btn-group hidden-xs" id="tableToolbar" role="group">
	                              ...
	                              <table 
	                              	id="table" 
	                              	data-toggle="table" 
	                              	data-url="${basePath}/eval/evalDistribute/getEvalDistributeList" 
					            ...
					            <thead>
						            <tr>
	                                       ...
	                                       <th data-field="znames" class="col-sm-4">测评员工</th>
          ...
          <div class="modal-dialog">
              <div class="modal-content animated bounce">
                  <div class="modal-header">
                      ...
                      <h5 class="modal-title">测评分配</h5>
                  </div>
                  <div class="modal-body">
                   	<div class="ibox-content">
                       <ul id="employeeTree" class="ztree"></ul>
		            </div>
                  </div>
   ...
   <!-- 全局js -->
	...
	function getZNodes(eid,rid){
		 ...
		 $.ajax({
    			cache: false,
    			type: "POST",
    			url: "${basePath }/system/employee/getEmployeeTreeCheck",	
    			data: {eid: eid,rid: rid},	
    			async: false,
    			success: function(data) {
    				zNodes = data;
    				$.fn.zTree.init($("#employeeTree"), setting, zNodes);
    				//$.fn.zTree.getZTreeObj("employeeTree").expandAll(true);
    			},
    ...
    function refresh(){
    	$("#table").bootstrapTable('refresh',{url: "${basePath}/eval/evalDistribute/getEvalDistributeList",query: {offset: pa.offset}});
    }
    ...
    window.operateEvents = {
        ...
        function operateFormatter(value, row, index) {
            return [
                ...
                'data-toggle="modal" data-target="#authorityModal"><i class="glyphicon  glyphicon-edit"></i>&nbsp;&nbsp;分配员工</button>',
                '</div>'
            ].join('');
        }
    ...
    $(function(){
    	$("#confirmDmMod").click(function(){
    		var nodes = $.fn.zTree.getZTreeObj("employeeTree").getCheckedNodes(true);
    		var eeidsArr = new Array();
            for(var i = 0; i < nodes.length; i++) {
            	if(!nodes[i].isParent){//只留下叶节点
            		eeidsArr.push(nodes[i].id);
            	}
            }
            var eeids = eeidsArr.join(",");
            $.ajax({
	     			cache: false,
	     			type: "POST",
	     			url: "${basePath}/eval/evalDistribute/addEvalDistribute",	
	     			data: {eid: eid,rid: rid,eeids: eeids},	
	     			async: false,
	     			success: function(data) {
	     				$("#closeDmMod").click();
	     				swal({title:"测评员工",text:"分配成功!",timer:2000, type: "success"});
	     				refresh();
	     			},
