...
<body class="gray-bg">
							...
							<div class="btn-group hidden-xs" id="tableToolbar" role="group">
								<button type="button" class="btn btn-outline btn-primary" onclick="javascript:history.go(-1);" >
	                          	...
	                          	<c:if test="${fn:contains(sessionScope.rName,'管理员')==true
		                          	...
		                          	<a href="${basePath}/result/departmentResult/exportDResult?sid=${sid}&epType=1" class="btn btn-outline btn-primary"><i class="glyphicon glyphicon-export"></i>&nbsp;导出加权汇总</a>
		                          	<a href="${basePath}/result/departmentResult/exportDResult?sid=${sid}&epType=0" class="btn btn-outline btn-primary"><i class="glyphicon glyphicon-export"></i>&nbsp;导出原始汇总</a>
	                          	</c:if>
	                         </div>
	                              <table 
	                              	id="table" 
	                              	data-toggle="table" 
	                              	data-url="${basePath}/result/departmentResult/getDList" 
	                              	data-query-params="queryParams"
	                              >
					            <thead>
						            <tr>
						                <th data-field="did" class="col-sm-1">编号</th>
	                                    <th data-field="dtname" class="col-sm-3">类型</th>
   ...
   <!-- 全局js -->
    ...
    function refresh(){
    	$("#table").bootstrapTable('refresh',{url: "${basePath}/result/departmentResult/getDList"});
    }
    window.operateEvents = {
    	         'click .select': function (e, value, row) {
	    	        	 layer.open({
	    	                 type: 2,
	    	                 title: ['&nbsp;&nbsp;'+row.dname+'(部门)测评结果', 'text-align:center;margin:0 auto;font-size:24px;'],
	    	                 shadeClose: false,
	    	                 shade: [0.3, '#393D49'],
	    	                 fix: true,
	    	                 //maxmin: true, //开启最大化最小化按钮
	    	                  area: ['1180px', '630px'],
	    	                 //closeBtn: 2,
	    	                 content: '${basePath}/result/departmentResult/getDResult?did='+row.did+"&sid="+${sid}
	    	             });
    	        }
        };
        ...
        function operateFormatter(value, row, index) {
            return [
                ...
                '<a  class="btn export btn-primary btn-sm btn-outline" href="${basePath}/result/departmentResult/exportDResult?sid=${sid}&epType=0&did='+row.did+'" >',
