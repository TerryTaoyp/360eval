...
<body class="gray-bg">
	                    ...
	                    <div class="ibox-content">
							<div class="btn-group hidden-xs" id="tableToolbar" role="group">
								<button type="button" class="btn btn-outline btn-primary" onclick="javascript:history.go(-1);" >
	                          	...
	                          	<a href="${basePath}/result/employeeResult/exportEDResult?sid=${sid}&epType=1" class="btn btn-outline btn-primary"><i class="glyphicon glyphicon-export"></i>&nbsp;导出加权汇总</a>
	                          	<a href="${basePath}/result/employeeResult/exportEDResult?sid=${sid}&epType=0" class="btn btn-outline btn-primary"><i class="glyphicon glyphicon-export"></i>&nbsp;导出原始汇总</a>
	                          	</c:if>
	                         </div>
	                              <table 
	                              	id="table" 
	                              	data-toggle="table" 
	                              	data-url="${basePath}/result/employeeResult/getEdList" 
   ...
   <!-- 全局js -->
    ...
    function refresh(){
    	$("#table").bootstrapTable('refresh',{url: "${basePath}/result/employeeResult/getEdList"});
    }
    window.operateEvents = {
    	         'click .select': function (e, value, row) {
	    	        	 layer.open({
	    	                 type: 2,
	    	                 title: ['&nbsp;&nbsp;'+row.dname+'—'+row.zname+'(部门经理)测评结果', 'text-align:center;margin:0 auto;font-size:24px;'],
	    	                 shadeClose: false,
	    	                 shade: [0.3, '#393D49'],
	    	                 fix: true,
	    	                 //maxmin: true, //开启最大化最小化按钮
	    	                  area: ['1180px', '630px'],
	    	                 //closeBtn: 2,
	    	                 content: '${basePath}/result/employeeResult/getEdResult?eid='+row.eid+"&sid="+${sid}
	    	             });
    	        }
        };
        ...
        function operateFormatter(value, row, index) {
            return [
                ...
                '<a  class="btn export btn-primary btn-sm btn-outline" href="${basePath}/result/employeeResult/exportEDResult?sid=${sid}&epType=0&eid='+row.eid+'" >',
